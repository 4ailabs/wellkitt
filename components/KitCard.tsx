...
const KitCard: React.FC<KitCardProps> = ({ kit, allProducts, onShowDetails }) => {
  ...
  return (
    <motion.div
      className={`${color.bg} border border-gray-200 rounded-xl md:rounded-2xl shadow-lg hover:shadow-2xl hover:scale-[1.03] transition-all duration-300 overflow-hidden flex flex-col h-full min-h-[400px] md:min-h-[480px]`}
      ...
      <div className="p-4 md:p-6 flex-grow">
        <div className="flex items-start justify-between mb-3 md:mb-4">
            <div className={`p-2 md:p-3 bg-white rounded-full`}>
                {kitIcons[kit.id]
                  ? React.cloneElement(kitIcons[kit.id] as React.ReactElement, { className: `w-6 h-6 md:w-8 md:h-8 ${color.icon}` })
        ...
        <h3 className="text-lg md:text-2xl font-extrabold text-slate-900 mb-3 md:mb-4 tracking-tight leading-snug" style={{ fontFamily: 'Montserrat, sans-serif' }}>{kit.name}</h3>
        <p className="text-sm md:text-base text-slate-700 mb-4 md:mb-6 h-10 md:h-12 leading-relaxed">{kit.benefit}</p>
